...
const Button = ({
  ...
  const overlayVariants: Record<"white" | "black" | "orange", string> = {
    ...
    orange:
      "absolute inset-0 bg-red-600 transition-transform duration-500 group-hover:scale-x-0 origin-center",
  };
  ...
  if (variant) {
    return (
      <div onClick={buttonEvent}>
        <Link
          href={`/${href}`}
          className={` group relative inline-flex items-center justify-center px-6 py-4 text-sm font-semibold uppercase tracking-widest overflow-hidden transition-colors ease-in-out duration-500 ${variants[variant]}`}
        >
